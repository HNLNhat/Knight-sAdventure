...
const updateCoin = async (name, coin) => {
    try {
        return await productService.updateCoinByNameUser(name, coin);
    } catch (error) {
        console.log(error);
    }
...
const getCoin = async (name) => {
    try {
        return await productService.getCoinByUser(name);
    } catch (error) {
        console.log(error);
    }
...
const addPet = async (petname,price,mdescribe)=>{
    try {
        return await productService.addPet(petname, price, mdescribe);
    } catch (error) {
        console.log(error);
    }
}
...
const petinUser = async (id_user,id_pet)=>{
    try {
        return await productService.petinUser(id_user,id_pet);
    } catch (error) {
        console.log(error);
    }
}
...
module.exports = {petinUser, addPet,newProduct, getAllRank, updateProductById, getCoin, deleteProductByID, addProduct, getProductById, Savepoint, getAllUser, addnewProduct, Changname, updateCoin };