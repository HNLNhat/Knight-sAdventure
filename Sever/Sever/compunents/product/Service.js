const { User,PetinUser,Pet } = require('../Model');
...
const updateCoinByNameUser = async (name, coin) => {
  try {
    ...
    console.log('saii', error);
  }

}
const getCoinByUser = async (name) => {
  try {
    const user = await User.findOne({ name });
    console.log(user);
    if (user) return user.coin;
  } catch (error) {
    console.log('saii', error);
  }
}

const addPet = async (petname,price,mdescribe) => {
  try {
    const newPet = new Pet({petname,price,mdescribe});
    await newPet.save();
    return true;
  } catch (error) {
...
const petinUser = async (id_user,id_pet) => {
  try {
    const newpetinUser = new PetinUser({id_user,id_pet});
    await newpetinUser.save();
    return true;
  } catch (error) {
    console.log('loi', error);
    return false;
...
module.exports = {petinUser,addPet, getAllRank, addProduct, getProductById, Savepoint, getAllUsers, deleteProductByID, getCoinByUser, addnewProduct, Changname, updateCoinByNameUser };
