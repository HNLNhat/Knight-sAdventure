...
router.post('/addpet', async (req, res, next) => {
  try {
    const { petname, price, mdescribe } = req.body;
    const addnew = await productController.addPet(petname, price, mdescribe);
    if (addnew) {
...
router.post('/callpet', async (req, res, next)=>{
try {
  const {id_user,id_pet} = req.body;
  const callpet = await productController.petinUser(id_user,id_pet);
  if (callpet) {
    return res.status(200).json({ addnew: true });
  } else {
    return res.status(400).json({ addnew: false });
...
router.post('/updateCoin', async (req, res, next) => {
  try {
    ...
    const addnew = await productController.updateCoin(name, coin);
    if (addnew) {
      return res.status(200).json({
        status: 1,
        Notification: "lưu thành công",
...
router.get('/getCoin', async (req, res, next) => {
  try {
    const { name } = req.body;
    const coin = await productController.getCoin(name);
    if (coin) {
      return res.status(200).json({
        coin: coin,
      });
...
router.post('/create_payment_url', function (req, res, next) {
  ...
  req.session.coin = Number(req.body.xu);
  res.redirect(vnpUrl)
});
...
router.get('/vnpay_return', async function (req, res, next) {
  ...
  if (secureHash === signed) {
    ...
    await productController.updateCoin(name, coin);
